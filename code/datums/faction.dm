 ...
 *
 *  Map datums can optionally specify a list of /datum/faction paths. These paths
 *  are used to initialize the global factions object, which contains a list of
 *  branch objects the map uses. Each branch definition specifies a list of
 *  /datum/job_rank paths, which are ranks available to that branch.
 *
...
var/datum/factions/factions = new()
 ...
 */
/datum/factions/proc/get_branch(var/branch_name)
 ...
 */
/datum/factions/proc/get_branch_by_type(var/branch_type)
 ...
 */
/datum/factions/proc/get_rank(var/branch_name, var/rank_name)
	if(rank_name == "None")
		return null

	var/datum/faction/branch = get_branch(branch_name)

	if(!branch || !(rank_name in branch.ranks))
		return null

	var/datum/job_rank/rank = branch.ranks[rank_name]
	return rank

/**
 *  Return all spawn branches for the given input
 */
/datum/factions/proc/spawn_branches(var/datum/species/S)
	if(!S)
 ...
 */
/datum/factions/proc/spawn_ranks(var/branch_name, var/datum/species/S)
	var/datum/faction/branch = get_branch(branch_name)
 ...
 */
/datum/factions/proc/is_spawn_branch(var/branch_name, var/datum/species/S)
 ...
 */
/datum/factions/proc/is_spawn_rank(var/branch_name, var/rank_name, var/datum/species/S)
	var/datum/faction/branch = get_branch(branch_name)
...
/datum/faction
	...
	var/list/ranks // Associative list of full rank names to the corresponding
	               // /datum/job_rank objects. These are all ranks available to the branch.
...
/datum/faction/New()
	...
	for(var/rank_path in rank_types)
		if(!ispath(rank_path, /datum/job_rank))
			crash_with("[name]'s rank_types includes [rank_path], which is not a subtype of /datum/job_rank.")
			continue
		var/datum/job_rank/rank = new rank_path ()
		ranks[rank.name] = rank

		if(rank_path in spawn_rank_types)
			spawn_ranks_[rank.name] = rank

/datum/faction/proc/spawn_ranks(var/datum/species/S)
	if(!S)
...
/hook/startup/proc/populate_branches()
	if(!(GLOB.using_map.flags & MAP_HAS_BRANCH) && !(GLOB.using_map.flags & MAP_HAS_RANK))
		factions.branches  = null
		factions.spawn_branches_ = null
		factions.spawn_branches_by_species_ = null
		return 1

	factions.branches  = list()
	factions.spawn_branches_ = list()
	factions.spawn_branches_by_species_ = list()
	for(var/branch_path in GLOB.using_map.branch_types)
		if(!ispath(branch_path, /datum/faction))
			crash_with("populate_branches() attempted to instantiate object with path [branch_path], which is not a subtype of /datum/faction.")
			continue

		var/datum/faction/branch = new branch_path ()
		factions.branches[branch.name] = branch

		if(branch_path in GLOB.using_map.spawn_branch_types)
			factions.spawn_branches_[branch.name] = branch
