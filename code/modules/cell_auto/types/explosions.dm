...
/atom/movable/cell/explosion/process()
	...
	if(shouldProcess() && group.shouldProcess()) // If we have not aged at all
		if(!loc.Enter(src))
			if(canCache())
				addToCache()
		else
...
/atom/movable/cell/explosion/spread()
	for(var/direction in GLOB.cardinal)
		...
		if(checkTurf(T) && prob( spread_chance))
			new /atom/movable/cell/explosion(T, group)
...
/datum/ca_group/explosion/proc/processCache()
	for(var/atom/AM in ex_act_cache)
		AM.ex_act(ex_act_cache[AM])

	ex_act_cache.Cut()

/datum/ca_group/explosion/proc/decayExplosion()
